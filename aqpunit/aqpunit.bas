...
902 DIM QR(2),QR$(2):QC=LEN(QG$):SET SPEED 3:QZ=FRE(""):IF Q3=0 THEN SCREEN 3
904 IF FRE(2)=$BEFF THEN QX=-1:QM=-PEEK($BFF6):QG=PEEK($BFF7):QF=-PEEK($BFFE):QU=PEEK($BFFF):IF QU=0 THEN CLS
...
919 _assert_bool:GETARGS QV,QA$:GOSUB _quotes:GOTO 924
...
926 DOKE $38AF,$38B1:GOSUB _output:ARGS ERR$+" error in "+QA$
927 RESUME:ON ERROR GOTO 0:QV=0:QR(2)=QR(2)+1
