...
482 _pokewp:GETARGS PP,PA,PB:GOSUB _output:ARGS "POKE! @%%,%%,%%" % (PP,PA,PB):POKE! @PP,PA,PB:RETURN
484 _peekwp:GETARGS PP,PA,PB:GOSUB _assert:ARGS "PEEK!(@%%,%%)=%%" % (PP,PA,PB):RETURN
486 _peekwp_err:GETARGS PP,PA,PE:GOSUB _assert_err:ARGS "PEEK!(@%%,%%)" % (PP,PA),PE:RETURN
488 _pokewp_err:GETARGS PP,PA,PB,QE:QA$="POKE! @%%,%%,%%" % (PP,PA,PB):ON ERROR GOTO _yes_err:POKE! @PP,PA,PB:GOTO _no_err
