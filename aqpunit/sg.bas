...
314 PP=PEEKSCREEN(SC-1):PB=PEEKSCREEN(SC):PE=PEEKSCREEN(SE):PN=PEEKSCREEN(SE+1)
315 GOSUB _main_screen:GOSUB _output:ARGS L$
316 GOSUB _assert:ARGS "PP=' '"
317 GOSUB _assert:ARGS "PB='@'"
318 GOSUB _assert:ARGS "PE='@'"
319 GOSUB _assert:ARGS "PN=' '"
...
520 GOSUB _title:ARGS "LOCATE, PRINT @, CURSORX, CURSORY, CURSOROFFSET, and OFFSET()"
521 FOR SY=0 TO 23 STEP 2:SX=INT(SY*SS+1)
522 GOSUB _output:ARGS "SX=%% SY=%%" % (SX,SY)
523 GOSUB _output:ARGS LIST$(NEXT):GOSUB _work_screen
524 CO=OFFSET(SX,SY)
525 GOSUB _main_screen:GOSUB _output:ARGS LIST$(NEXT):GOSUB _work_screen
526 LOCATE SX,SY:CX=CURSORX:CY=CURSORY:CF=CURSOROFFSET
527 GOSUB _main_screen:GOSUB _output:ARGS LIST$(NEXT):GOSUB _work_screen
528 PRINT @(SX+1,SY+1);:AX=CURSORX:AY=CURSORY:AF=CURSOROFFSET

530 IF Y=23 THEN GOSUB _getkey
531 GOSUB _main_screen
532 GOSUB _assert:ARGS "CX=%%" % (SX)
533 GOSUB _assert:ARGS "CY=%%" % (SY)
534 GOSUB _assert:ARGS "CF=%%" % (SY*SC+SC+SX)
535 GOSUB _assert:ARGS "AX=%%" % (SX+1)
536 GOSUB _assert:ARGS "AY=%%" % (SY+1)
537 GOSUB _assert:ARGS "AF=%%" % (SY*SC+SC*2+SX+1)
538 GOSUB _assert:ARGS "CO=%%" % (SY*SC+SC+SX)
539 NEXT
